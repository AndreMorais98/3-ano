...
void writeXML(string f){
    ifstream old("./../config.xml");
    ofstream tmp("./../tmp.xml");
    tmp << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" << endl;
    tmp << "<scene>" << endl;
    string g;
    while (getline(old,g)){
        if (g.compare("</scene>") != 0 && g.compare("<scene>") !=0 && g.compare("<?xml version=\"1.0\" encoding=\"UTF-8\"?>") !=0){
            tmp << g << endl;
        }
    }
    tmp << " <model file=\"" << f << "\"/>" << endl;
    tmp << "</scene>" << endl;
    old.close();
    tmp.close();
    remove("./../config.xml");
    rename("./../tmp.xml","./../config.xml");
}
