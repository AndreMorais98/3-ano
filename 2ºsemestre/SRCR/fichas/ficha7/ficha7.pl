...
%Invariantes
%--------------------------------- - - - - - - - - - -  -  questão x
+jogo(J,A,C) ::	(solucoes(Arbitros ,jogo(I,Arbitros,_),S), comprimento(S,N), N ==1).


%--------------------------------- - - - - - - - - - -  -  questão xi
+jogo(J,A,C) :: (solucoes(Jogos ,jogo(Jogos,A,_),S), comprimento(S,N),N<=3).


%--------------------------------- - - - - - - - - - -  -  questão xii
+jogo(J,A,C) :: (solucoes(Jogos ,jogo(Jogos,A,_),S), nao(seguido(S))).


head([H|T],H).

-seguido([]).
seguido([H|T]) :- head(T,X), H =:= X-1.
seguido([H|T]):- head(T,X), H \= X-1, seguido(T).
